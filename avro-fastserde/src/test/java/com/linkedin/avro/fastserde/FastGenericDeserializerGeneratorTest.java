...
public class FastGenericDeserializerGeneratorTest {
    ...
    public void shouldReadSplittedAndAliasedSubRecordFields(Implementation implementation) {
        ...
        GenericRecord builder = new GenericData.Record(splittedAndAliasedRecordSchema);
        builder.put("record1", aliasedSubRecordBuilder);
        builder.put("record2", subRecordBuilder);

        List<GenericRecord> recordArray = new ArrayList<>();
        recordArray.add(aliasedSubRecordBuilder);
        builder.put("recordArray", recordArray);

        // when
        GenericRecord record = implementation.decode(splittedAndAliasedRecordSchema, recordSchema, genericDataAsDecoder(builder));

        // then
        Assert.assertEquals(new Utf8("abc"), ((GenericRecord) record.get("record1")).get("subField"));
        Assert.assertEquals(subRecordSchema.hashCode(), ((GenericRecord) record.get("record1")).getSchema().hashCode());
        Assert.assertEquals(new Utf8("abc"), ((GenericRecord) record.get("record2")).get("subField"));
        Assert.assertEquals(subRecordSchema.hashCode(), ((GenericRecord) record.get("record2")).getSchema().hashCode());
    }
